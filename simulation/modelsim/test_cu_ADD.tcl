...
proc verify_signals {time signals} {
	...
	check_signal state [dict get $signals state] $state
	check_signal pc_write [dict get $signals pc_write] $pc_write
	check_signal inst_data_sel [dict get $signals inst_data_sel] $inst_data_sel
	check_signal mem_read [dict get $signals mem_read] $mem_read
	check_signal mem_write [dict get $signals mem_write] $mem_write
	check_signal ir_write [dict get $signals ir_write] $ir_write
	check_signal rd_sel [dict get $signals rd_sel] $rd_sel
	check_signal rd_data_sel [dict get $signals rd_data_sel] $rd_data_sel
	check_signal rd_write_en [dict get $signals rd_write_en] $rd_write_en
	check_signal alu_op_a_sel [dict get $signals alu_op_a_sel] $alu_op_a_sel
	check_signal alu_op_b_sel [dict get $signals alu_op_b_sel] $alu_op_b_sel
	check_signal alu_arith_fn [dict get $signals alu_arith_fn] $alu_arith_fn
	check_signal alu_logic_fn [dict get $signals alu_logic_fn] $alu_logic_fn
	check_signal alu_fn [dict get $signals alu_fn] $alu_fn
	check_signal pc_src_sel [dict get $signals pc_src_sel] $pc_src_sel
	check_signal jmp_addr_sel [dict get $signals jmp_addr_sel] $jmp_addr_sel
}

proc check_signal {name expected real} {
	if {$expected != "x"} {
		if {$expected == $real} {
